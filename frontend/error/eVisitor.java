...
public class eVisitor {
    ...
    public eVisitor(ASTNode root, Error error) {
        this.root = root;
        this.error = error;
    }
    ...
    public void visit(ASTNode node) {
        ...
        if (node.isType("Stmt")) {
            eVisitorStmt eVisitorStmt = new eVisitorStmt(this);
            eVisitorStmt.visit(node);
        }
        else if (node.isType("Decl")) {
            eVisitorDecl eVisitorDecl = new eVisitorDecl(this);
            eVisitorDecl.visit(node);
        }
        else if (node.isType("FuncDef")) {
            eVisitorFuncDef eVisitorFuncDef = new eVisitorFuncDef(this);
            eVisitorFuncDef.visit(node);
        }
        else if (node.isType("MainFuncDef")) {
            Func = new Symbol(0,0,"main","func", "int");
            beforeBlock();
            eVisitorBlock eVisitorBlock = new eVisitorBlock(this);
