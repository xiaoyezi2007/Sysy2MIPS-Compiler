...
public class eVisitorExp {
    eVisitor eVisitor;

    public eVisitorExp(eVisitor eVisitor) {
    ...
    public void visitFuncCall(ASTNode node) {
        ...
        if (!eVisitor.pt.checkSymbol(funcName) && !funcName.equals("getint")) {
            eVisitor.error.addError("c", children.get(0).getToken().getLine());
            return;
        }
        if (children.get(2).isType("FuncRParams")) {
            ...
            if (!eVisitor.pt.getSymbol(funcName).noParam()) eVisitor.error.addError("d", children.get(0).getToken().getLine());
        }
    }
    ...
    public void visitFuncRParams(int line, String funcName, ASTNode node) {
        ...
        Symbol s = eVisitor.pt.getSymbol(funcName);
        if (funcName.equals("getint") || types.size() != s.getParamNum()) {
            eVisitor.error.addError("d", line);
            return;
        }
        if (!eVisitor.pt.getSymbol(funcName).checkParams(types)) {
            eVisitor.error.addError("e", line);
        }
    }
                    ...
                    if (children4.size()==1&&children4.get(0).isType("PrimaryExp")) {
                        ...
                        if (children5.size() == 1 && children5.get(0).isType("LVal")) {
                            ...
                            if (children6.size()==1&&children6.get(0).isType("IDENFR")) {
                                ...
                                if (eVisitor.pt.checkSymbol(name) && eVisitor.pt.getSymbol(name).isArray()) {
                                    return 1;
                                }
                            }
                        }
